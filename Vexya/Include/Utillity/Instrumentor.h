	...
	public:
		...
		void BeginSession(const std::string& name, const std::string& filepath = "results.json")
		{
			...
			if (m_CurrentSession)
			{
				...
				// profiling output.
				if (Log::EngineLogger()) // Edge case: BeginSession() might be before Log::Init()
				{
...
#define VX_PROFILE 1
	...
	#define VX_PROFILE_BEGIN_SESSION(name, filepath) ::Vexya::Instrumentor::Get().BeginSession(name, filepath)
	#define VX_PROFILE_END_SESSION() ::Vexya::Instrumentor::Get().EndSession()
	#define VX_PROFILE_SCOPE_LINE2(name, line) constexpr auto fixedName##line = ::Vexya::InstrumentorUtils::CleanupOutputString(name, "__cdecl ");\
												   ::Vexya::InstrumentationTimer timer##line(fixedName##line.Data)
